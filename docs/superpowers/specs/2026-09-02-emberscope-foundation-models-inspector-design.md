...
20. `EmberScope` builds warnings-as-errors under `SWIFT_STRICT_CONCURRENCY = complete` (0 warnings), `EmberScopeTests` under complete concurrency too, and the standalone extraction was smoke-tested for real (`swift build && swift test`, Swift 6 language mode) rather than asserted. *(Task 0 deferred item; final review B7/C6)*
22. **Distribution (2026-09-08, user decision):** a root `Package.swift` exposing only the `EmberScope` library makes `https://github.com/Vvlladd/Ember` installable as a Swift package, netfox-style, without extracting a second repository. Verified: `tuist generate` ignores the root manifest (same targets and schemes), `swift build && swift test` pass at the root (112 tests), and a separate consumer package can `import EmberScope` from the repo path. The earlier "no nested Package.swift" rule still holds for every directory below the root.

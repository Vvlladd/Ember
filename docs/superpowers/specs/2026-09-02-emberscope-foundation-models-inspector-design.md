...
| Default enablement | Enabled when the library is compiled with `DEBUG`, disabled otherwise; hosts are told to also wrap `start()` in `#if DEBUG` (netfox guidance). When disabled every wrapper is a zero-cost pass-through. | Shippable without fear; still opt-in-able for internal builds via `ScopeConfiguration(isEnabled: true)`. |

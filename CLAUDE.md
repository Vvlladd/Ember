...
- **Tuist scheme autogeneration groups by name suffix, and "Example" is a RUN suffix** (alongside "Demo"), so `EmberScopeExample` was silently folded into the `EmberScope` scheme as its run target instead of getting one of its own. `Project.swift` therefore states `automaticSchemesOptions` explicitly — Tuist's own defaults with `"Example"` dropped from `run`. If you add a target whose name ends in `App`, `Demo`, `Implementation`, `Interface`, `Mocks`, `Testing`, `Tests`, `UITests` or `SnapshotTests`, check `Ember.xcodeproj/xcshareddata/xcschemes/` after generating: it may not have the scheme you expect.

...
- **EmberScope must never import `FoundationChatKit`** — the dependency runs the other way, which is what keeps the library liftable into another project. `ScopeConfiguration.isEnabled` defaults to the library's own `DEBUG`, and when it is off **every wrapper records nothing and allocates no per-request state** — preserve that in any new wrapper method. No disk, no network: the ring buffer is the only storage. `EmberScope.note(…)` carries counts and kinds, **never user text** — note text is the one string `captureContent: false` does NOT redact (notes are developer annotations). Every **app-side** EmberScope surface is behind `#if DEBUG`, so a Release build of Ember has none of it; the framework itself stays linked and inert (the provider creates sessions through it unconditionally). The EmberScope target builds warnings-as-errors under `SWIFT_STRICT_CONCURRENCY = complete`. The **root `Package.swift` exposes only the `EmberScope` library** so any app can add this repo URL as a Swift package (netfox-style); `tuist generate` ignores it (verified: identical targets and schemes) and Tuist's own dependency manifest stays in `Tuist/Package.swift`. Keep the root manifest's `path:` entries pointing at `Targets/EmberScope/{Sources,Tests}/…`, keep it dependency-free, and never add a `Package.swift` anywhere else in the tree. Gate: `swift build && swift test` at the repo root. Presentation (`EmberScope.present()`, shake, ⌘⇧E) gates on `configuration.isEnabled` only — never on the recording toggle, so a paused inspector stays openable. `ScopeStore.refresh()` is `async` (the fold runs off the main actor behind a generation guard); error chains are stored root first; `streamResponse`/`collect()` return `sending` values like the SDK. The full list of rulings, deferred items and follow-ups is the **Decision log** in the EmberScope spec (`docs/superpowers/specs/2026-09-02-emberscope-foundation-models-inspector-design.md`) — extend it when you change a decision.
...
- **Two Xcodes, two SDKs.** The command line uses Xcode 26.6 (`xcode-select`), while the IDE may be the Xcode 27 beta (Swift 6.4, macOS/iOS 27 SDK). `EmberScope` builds **warnings-as-errors**, and the 27 SDK renamed `GenerationOptions.sampling` → `samplingMode` (old name deprecated) and added `Transcript.Segment.attachment` / `Transcript.Entry.reasoning`. Such SDK deltas are handled with `#if compiler(>=6.4)` in `TranscriptSnapshot.swift` — keep that pattern; never accept an Xcode fix-it that leaves the code compiling on only one SDK (a `samplingMode` fix-it broke the 26.6 build once). Verify both: the normal gates, plus `DEVELOPER_DIR=/Applications/Xcode-27.0.0-Beta.5.app/Contents/Developer xcodebuild -workspace Ember.xcworkspace -scheme EmberScope -destination 'platform=macOS' -derivedDataPath /tmp/dd-beta build`.
- **Xcode's "Update to recommended settings" on the generated projects** cannot be answered with "Perform Changes" (the `.xcodeproj` files are git-ignored and regenerated). The durable fix already in place: the project-level `xcodeRecommended` settings in `Project.swift`, the same keys in `Tuist/Package.swift` for the SPM-generated dependency projects, and `Workspace.swift`'s `lastXcodeUpgradeCheck` stamp. If Xcode flags a new setting, add it there, not in Xcode.
